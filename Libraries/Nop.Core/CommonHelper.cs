...
namespace Nop.Core
{
    ...
    public partial class CommonHelper
    {
        ...
        public static string EnsureSubscriberPhonelOrThrow(string phone)
        {
            string output = EnsureNotNull(phone);
            output = output.Trim();
            output = EnsureMaximumLength(output, 12);

            if (!IsValidIsraelPhone(output))
            {
                throw new NopException("phone is not valid.");
            }
        ...
        public static bool IsValidIsraelPhone(string phone)
        {
            if (String.IsNullOrEmpty(phone))
                return false;

            phone = phone.Trim();
            var result = Regex.IsMatch(phone, @"(\d{3}-?\d{7})", RegexOptions.IgnoreCase);
            return result;
        }
