...
namespace Nop.Services.Messages
{
    ...
    public class NewsLetterSubscriptionService : INewsLetterSubscriptionService
    {
        ...
        public virtual void UpdateNewsLetterSubscription(NewsLetterSubscription newsLetterSubscription, bool publishSubscriptionEvents = true)
        {
            ...
            newsLetterSubscription.phone = CommonHelper.EnsureSubscriberEmailOrThrow(newsLetterSubscription.phone);

            //Handle e-mail
            if (!string.IsNullOrWhiteSpace(newsLetterSubscription.Email))
            {
                newsLetterSubscription.Email = CommonHelper.EnsureSubscriberEmailOrThrow(newsLetterSubscription.Email);
            }
        ...
        public virtual NewsLetterSubscription GetNewsLetterSubscriptionByPhoneAndStoreId(string phone, int storeId)
        {
            ...
            var newsLetterSubscriptions = from nls in _subscriptionRepository.Table
                                          where nls.phone == phone && nls.StoreId == storeId
                                          orderby nls.Id
                                          select nls;
        ...
        public virtual IPagedList<NewsLetterSubscription> GetNewsLetterSubscriptionByNameAndStoreId(string name, int storeId, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            ...
            var newsLetterSubscriptions = from nls in _subscriptionRepository.Table
                                          where nls.name == name && nls.StoreId == storeId
                                          orderby nls.Id
                                          select nls;


            var subscriptions = new PagedList<NewsLetterSubscription>(newsLetterSubscriptions, pageIndex, pageSize);
        ...
        public virtual IPagedList<NewsLetterSubscription> GetAllNewsLetterSubscriptions(string phone = null,string email = null,
            ...
            int pageIndex = 0, int pageSize = int.MaxValue)
        {
            if (customerRoleId == 0)
            {
                ...
                if (!String.IsNullOrEmpty(phone))
                    query = query.Where(nls => nls.phone.Contains(phone));
                ...
                query = query.OrderByDescending(nls => nls.CreatedOnUtc);
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (!String.IsNullOrEmpty(phone))
                        query = query.Where(nls => nls.NewsletterSubscribers.phone.Contains(phone));
                    if (!String.IsNullOrEmpty(email))
