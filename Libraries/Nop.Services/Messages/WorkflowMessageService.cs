...
namespace Nop.Services.Messages
{
    public partial class WorkflowMessageService : IWorkflowMessageService
    {
        ...
        public virtual int SendOrderPlacedStoreOwnerNotification(Order order, int languageId)
        {
            ...
            var toEmail = _storeContext.CurrentStore.CompanyMail; 
            var toName = emailAccount.DisplayName;
        ...
        public virtual int SendOrderPaidStoreOwnerNotification(Order order, int languageId)
        {
            ...
            try
            {
                if (store.SendSmsOnOrderToOwner && !string.IsNullOrWhiteSpace(store.OwnerPhoneNumber) && !string.IsNullOrWhiteSpace(store.SmsPaidOrderMsg) && !string.IsNullOrWhiteSpace(store.SmsUserName) && !string.IsNullOrWhiteSpace(store.SmsPassword) && !string.IsNullOrWhiteSpace(store.SmsSender))
                {
                    string msg3 = store.SmsPaidOrderMsg.Replace("[מספר הזמנה]", order.Id.ToString());
                    _emailsender.SendSmS(store.OwnerPhoneNumber, msg3, store.SmsUserName, store.SmsPassword, "", store.SmsSender);
                }
        ...
        public virtual void SendNewsLetterSubscriptionMsgForAdmin(NewsLetterSubscription subscription, int languageId)
        {
            if (subscription == null)
                throw new ArgumentNullException("subscription");
            ...
            var messageTemplate = GetActiveMessageTemplate(MessageTemplateSystemNames.NewsletterSubscriptionMsgForAdmin, store.Id);
            if (messageTemplate == null)
                return;
            ...
            //event notification
            //_eventPublisher.MessageTokensAdded(messageTemplate, tokens);
            string newLeadMsg = store.newLeadMsg.Replace("%NAME%", subscription.name);
             newLeadMsg = newLeadMsg.Replace("%PHONE%", subscription.phone);
            newLeadMsg = newLeadMsg.Replace("%SUB%", subscription.subject);


            _emailsender.SendEmail(emailAccount, store.newLeadSubject, newLeadMsg, emailAccount.Email, emailAccount.DisplayName, store.CompanyMail, store.CompanyName);
        ...
        public virtual int SendContactUsMessage(int languageId, string senderEmail,
            string senderName, string subject, string body,string senderPhone)
        {
            var store = _storeContext.CurrentStore;
            languageId = EnsureLanguageIsActive(languageId, store.Id);
            ...
            var tokens = new List<Token>();
            _messageTokenProvider.AddStoreTokens(tokens, store, emailAccount);
            tokens.Add(new Token("ContactUs.SenderPhone", senderPhone));
            ...
            var toEmail = _storeContext.CurrentStore.CompanyMailLeads;
            var toName = emailAccount.DisplayName;
