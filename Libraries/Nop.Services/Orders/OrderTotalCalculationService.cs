...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderTotalCalculationService : IOrderTotalCalculationService
    {
        ...
        #endregion
        decimal AddresPriceAdjustGlobal = decimal.Zero;///////////////////////// i did it
            ...
            out SortedDictionary<decimal, decimal> taxRates)
        {
            ...
            decimal AddresPriceAdjust = decimal.Zero;/////////////////////
            /////Address attributes
            if ((customer != null && customer.ShippingAddress != null) && customer.ShippingAddress.CustomAttributes != null)
            {
                string shippingAddressAttXml = customer.ShippingAddress.CustomAttributes;

                var shippingAddressPrice = _addressAttributeParser.ParseAddressAttributeValues(shippingAddressAttXml);
                if (shippingAddressPrice != null)
                {
                    foreach (var sap in shippingAddressPrice)
                    {
                        AddresPriceAdjust += sap.PriceAdjustment;

                    }
                    AddresPriceAdjustGlobal = AddresPriceAdjust;
            ...
            out decimal taxRate, out List<DiscountForCaching> appliedDiscounts)
        {
            ...
            return shippingTotalTaxed + AddresPriceAdjustGlobal;
        }
